...
Confirm onchainos skills are present (e.g. `okx-dex`, `okx-dex-swap`, `okx-agentic-wallet`). If they do not appear, re-run the symlink command above and check `ls -la skills/` and `ls ~/.onchainos/skills/`. If still missing, re-run setup from Step 1.
