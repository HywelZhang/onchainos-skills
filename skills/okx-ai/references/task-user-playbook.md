...
**New-device default routing (login only).** Before the login heartbeat, the CLI checks whether this device already exists in the complete device table. A genuinely new device gets a durable local pending marker, is registered, then is added to every subscription's explicit `deviceList` by fresh-list union and batched overwrite (≤100 items per request); `deviceList:null` remains null because it already means default-all. The marker is cleared only after every required batch succeeds, so an interrupted/partially successful fan-out is retried idempotently on the next login. The CLI returns `postLoginSubscriptions` only after routing succeeds, so the table never appears before the new device is configured. An already-registered device without that pending marker is never rewritten on re-login, preserving any manual per-task opt-out. If the pre-heartbeat device probe is unavailable, the CLI skips both registration and the table rather than losing new-device detection. This automatic mutation does **not** run for `wallet status --include-subscriptions`.
