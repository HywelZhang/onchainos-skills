...
- **Roles:** `user` → **User** / 用户 · `asp` → **ASP** / 服务提供商 · `evaluator` → **Evaluator** / 评审员. Never show the raw enum token, never legacy nouns (buyer/seller), never a bilingual parenthetical.
