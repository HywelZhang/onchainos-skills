...
#[cfg(test)]
mod tests {
    use super::communication_help_supports_offline_replay;

    #[test]
    fn detects_offline_replay_capability_marker() {
        assert!(communication_help_supports_offline_replay(
            "Capabilities:\n  message-eligible-is-offline-replay\n"
        ));
        assert!(!communication_help_supports_offline_replay(
            "okx-a2a 0.1.11\nUsage: okx-a2a <command>\n"
        ));
    }
}
