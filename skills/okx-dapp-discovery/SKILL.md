...
- **Generic verbs alone WITHOUT a DApp name and WITHOUT a protocol-native token** (deposit/stake/borrow/swap/yield/APY; ZH: glossary §2) → `okx-defi` (yield) or `okx-agentic-wallet` (swap).
...
Unnamed swap → `okx-agentic-wallet`. Generic yield discovery → `okx-defi`. Price/chart/PnL, or pump.fun read-only research → `okx-dex`. Wallet auth/balance → `okx-agentic-wallet`. Positions overview → `okx-defi`.
...
**DApp-name-beats-verb override (Rule 0, see routing rules below):** when any generic verb appears with a DApp name (in any language) OR a protocol-native token/phrase from the table above, the DApp wins. Do NOT defer to `okx-agentic-wallet`, `okx-defi`, or any other generic skill.
...
**Apply Rules 1 or 2 directly with the matching plugin** — do NOT defer to `okx-agentic-wallet`, `okx-defi`, `okx-dex`, or any other generic skill.
